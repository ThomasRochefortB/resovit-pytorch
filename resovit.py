...
# Creates a vision transformer using the PatchEmbedding and PositionalEmbedding classes.
    ...
    def __init__(self,
        ...
        self.patch_embedding = PatchEmbedding(patch_size=patch_size,max_length=max_length, emb_dim=embedding_dim,img_channels=img_channels)
        self.embedding_dropout = nn.Dropout(p=embedding_dropout)
        self.positional_embedding = PositionalEmbedding(emb_dim=embedding_dim,max_length=max_length)

        self.transformer_encoder = nn.TransformerEncoder( 
            nn.TransformerEncoderLayer(d_model=embedding_dim, nhead=num_heads, dim_feedforward=mlp_size, dropout=attn_dropout),
    ...
    def forward(self, x):
        ...
        for i, img in enumerate(x):
            ...
            img = img.unsqueeze(0)
            patches, attn_mask = self.patch_embedding(img)
